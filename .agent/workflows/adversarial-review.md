...
description: Use before completing or committing any change — especially to scoring/classification/dashboard generation, the Makefile, or code that consumes scraped job records — and before any deploy (bash deploy-cloudrun.sh acaa-agent hits Cloud Run production). Attack your own change: lying type annotations, stale call sites, missing keys, and silent data loss.
---

# Adversarial Review Workflow

## Trigger Conditions
- Before marking any task complete on files touching `scoring_engine.py`, `stream_classifier.py`, `reclassify_jobs.py`, `deduplicate_jobs.py`, `build_categorized_dashboard.py`, the `Makefile`, or `job-dashboard/backend/src/job_dashboard/`.
- Before making any git commit (`/home/s/.openclaw/workspace` or submodules).
- ALWAYS before running `bash deploy-cloudrun.sh acaa-agent` or `bash deploy-cloudrun.sh acaa-agent-draft` (which deploys to Cloud Run production).
...
5. **Execute test and lint gauntlet**:
   - site: `cd /home/s/.openclaw/workspace/job-dashboard/backend && ruff check . --exclude _archive/ && python3 -m pytest tests/ -v`
   - modular: `cd /home/s/.openclaw/workspace/job-dashboard/backend && ruff check . && python3 -m pytest`
...
**Observation**: `job-dashboard/backend/reclassify_jobs.py:250` declares:
```python
...
- Deployment reviews: `bash deploy-cloudrun.sh acaa-agent` modifies Cloud Run production environment; requires explicit user authorization every time.
