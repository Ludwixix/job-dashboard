...
- Adding or modifying features in `job-dashboard/backend/src/job_dashboard/`.
...
1. **Pin the contract first**: Define precise input and output shapes using existing fixtures:
   - `job-dashboard/backend`: `sample_job` in `tests/conftest.py` (`id`, `title`, `company`, `location`, `url`, `source`, `score`, `fit`, `work_type`, `posted`, `matched_terms`, `gaps`, `why`).
   - `job-dashboard/backend`: Pydantic models from `src/job_dashboard/models.py` (`Job`, `ApplicationRecord`).
2. **Write the failing test first**:
   - `job-dashboard/backend`: Add test case under `tests/test_*.py`.
   - `job-dashboard/backend`: Add test case under `tests/test_*.py` (runs with `pythonpath = ["src"]` configured in `pyproject.toml`).
...
2. *Write test*: Add to `job-dashboard/backend/tests/test_reclassify.py`:
   ```python
   ...
   ```
3. *Run test and watch fail*: `cd /home/s/.openclaw/workspace/job-dashboard/backend && python3 -m pytest tests/ -v`.
...
6. *Note*: Re-run `npm run build` before generating final HTML.
...
- Rebuilding dashboard outputs (`index.html`, `index_categorized.html`) or deploying to Cloud Run: requires separate user approval.
