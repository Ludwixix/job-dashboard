...
description: Use whenever something fails or behaves unexpectedly in this workspace — a pytest failure or collection error, a traceback from any job-dashboard script (npm run build, python -m job_dashboard.run_server, server.py, scrape_daily.py), a wrong score/classification in the generated dashboard, or before proposing any fix at all. Debug first, fix second; never patch symptoms.
---
...
- Any traceback or non-zero exit from `npm run build`, `python -m job_dashboard.run_server`, `make dedup`, `make reclassify`, `server.py`, `build_categorized_dashboard.py`, `deduplicate_jobs.py`.
...
1. **Reproduce reliably**: Run the exact failing command and capture complete output:
   - site: `cd /home/s/.openclaw/workspace/job-dashboard/backend && python3 -m pytest tests/ -v` (or `npm run build`)
   - modular: `cd /home/s/.openclaw/workspace/job-dashboard/backend && python3 -m pytest`
   Ensure reliable reproduction before attempting any code change.
2. **Read the error bottom-up**: Anchor on the first stack frame inside workspace code (`job-dashboard/backend/*.py` or `job-dashboard/backend/src/job_dashboard/*.py`). Third-party library frames below it are context.
...
**Scenario**: `cd /home/s/.openclaw/workspace/job-dashboard/backend && python3 -m pytest` fails during collection:
```
...
1. *Reproduction*: Run `cd /home/s/.openclaw/workspace/job-dashboard/backend && python3 -m pytest` — reproduced immediately with 1 error during collection.
