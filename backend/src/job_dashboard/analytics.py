...
class JobAnalytics:
    ...
    def analyze_market_trends(
        self, jobs: list[JobDict], timeframe_days: int = 30
    ) -> dict[str, Any]:
        """
        ...
        if not jobs:
            return {"error": "No jobs to analyze", "trends": {}, "summary": {}}
        ...
        trends = {
            ...
            "salary_distribution": self._analyze_salary_distribution(
                [job for _, job in recent_jobs]
            ),
            "remote_work_trend": self._analyze_remote_work_trend(recent_jobs),
            "company_distribution": self._analyze_company_distribution(
                [job for _, job in recent_jobs]
            ),
        }
        ...
        summary = self._generate_market_summary(
            trends, len(recent_jobs), timeframe_days
        )
    ...
    def _calculate_daily_postings(
        self, dated_jobs: list[tuple[datetime, JobDict]]
    ) -> dict[str, int]:
    ...
    def _calculate_weekly_trend(
        ...
        if len(counts) >= 2:
            ...
            percentage_change = (
                ((counts[-1] - counts[-2]) / counts[-2] * 100) if counts[-2] > 0 else 0
            )
        else:
    ...
    def _calculate_top_skills(
        self, jobs: list[JobDict], top_n: int = 10
    ) -> list[dict[str, Any]]:
    ...
    def _analyze_salary_distribution(self, jobs: list[JobDict]) -> dict[str, Any]:
        ...
        if not salaries:
            return {"error": "No salary data available", "count": 0, "distribution": {}}
    ...
    def _analyze_remote_work_trend(
        ...
        for week in sorted(total_by_week.keys()):
            ...
            remote_percentages[week] = (
                round((remote / total) * 100, 1) if total > 0 else 0
            )
        ...
        return {
            ...
            "current_remote_pct": remote_percentages[
                list(remote_percentages.keys())[-1]
            ]
            if remote_percentages
            else 0,
            "trend": trend,
            "average_remote_pct": round(np.mean(list(remote_percentages.values())), 1)
            if remote_percentages
            else 0,
        }
    ...
    def _generate_market_summary(
        self, trends: dict[str, Any], total_jobs: int, timeframe_days: int
    ) -> dict[str, Any]:
        ...
        summary_parts.append(
            f"Average of {daily_avg} jobs posted per day over {timeframe_days} days"
        )
        ...
        if weekly_trend.get("trend") == "increasing":
            summary_parts.append(
                f"Job postings increasing ({weekly_trend.get('percentage_change', 0)}% this week)"
            )
        elif weekly_trend.get("trend") == "decreasing":
            summary_parts.append(
                f"Job postings decreasing ({weekly_trend.get('percentage_change', 0)}% this week)"
            )
        ...
        if top_skills:
            ...
            summary_parts.append(
                f"Most in-demand skill: {top_skill} ({top_percentage}% of jobs)"
            )
    ...
    def analyze_skills_gap(
        self, profile_skills: list[str], job_skills: list[list[str]], top_n: int = 5
    ) -> dict[str, Any]:
        """
        ...
        covered_skills = sum(
            count
            for skill, count in market_skill_counter.items()
            if skill in profile_skills_set
        )

        coverage_percentage = (
            (covered_skills / total_market_skills * 100)
        ...
        result = {
            ...
            "skill_gap_score": self._calculate_skill_gap_score(
                coverage_percentage, len(top_missing_skills)
            ),
            "recommendations": self._generate_skill_recommendations(
                top_missing_skills, coverage_percentage
            ),
        }
    ...
    def _calculate_skill_gap_score(
        self, coverage_percentage: float, missing_count: int
    ) -> float:
    ...
    def _generate_skill_recommendations(
        self, missing_skills: list[dict[str, Any]], coverage_percentage: float
    ) -> list[str]:
        ...
        if coverage_percentage >= 70:
            ...
            recommendations.append(
                "Consider developing 1-2 key missing skills to improve your competitiveness"
            )
        else:
            recommendations.append(
                "Significant skills gap identified - focus on developing in-demand skills"
            )
