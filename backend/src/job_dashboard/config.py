...
class Settings:
    ...
    def __init__(self):
        ...
        self.source_dir = self._get_path(
            "JOB_DASHBOARD_SOURCE_DIR", Path("Source of truth")
        )
        self.guidelines_dir = self._get_path(
            "JOB_DASHBOARD_GUIDELINES_DIR", Path("Guidelines")
        )
        ...
        self.seek_cache_fallback = self._get_bool(
        ...
        self.llm_model = os.getenv(
            "JOB_DASHBOARD_LLM_MODEL", "deepseek/deepseek-v4-flash-0731"
        )
        ...
        self.sync_interval_seconds = int(
            os.getenv("JOB_DASHBOARD_SYNC_INTERVAL_SECONDS", "1800")
        )
        ...
        self.proxy_list = [
            p.strip()
            for p in (os.getenv("JOB_DASHBOARD_PROXIES") or "").split(",")
            if p.strip()
        ]
        self.multi_board_enabled = self._get_bool(
            "JOB_DASHBOARD_MULTI_BOARD_ENABLED", False
        )
        self.stealth_browser_enabled = self._get_bool(
            "JOB_DASHBOARD_STEALTH_BROWSER_ENABLED", True
        )
