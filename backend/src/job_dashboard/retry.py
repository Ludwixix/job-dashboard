...
class RetryManager:
    ...
    def execute_with_retry(self, func: Callable[..., T], *args, **kwargs) -> T:
        ...
        for attempt in range(self.config.max_retries + 1):
            try:
                if attempt > 0:
                    self.logger.info(
                        f"Retry attempt {attempt}/{self.config.max_retries} for {func.__name__}"
                    )
                ...
                if attempt == self.config.max_retries:
                    self.logger.error(
                        f"All retries exhausted for {func.__name__}: {e!s}"
                    )
                    raise
    ...
    def create_decorator(self, config: RetryConfig | None = None):
        ...
        Returns:
            Decorator function
        """
        handler = RetryManager(config) if config else self
