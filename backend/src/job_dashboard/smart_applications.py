"""
Smart Application System for Phase 6
Advanced application tracking with AI-powered features
"""
...
class SmartApplicationTracker:
    ...
    def get_application(self, application_id: str) -> SmartApplication | None:
        """Get application by ID."""
        return self.applications.get(application_id)
    
    def get_applications_by_status(self, status: ApplicationStatus | None = None) -> list[SmartApplication]:
        """Get applications filtered by status."""
        if status is None:
            return list(self.applications.values())
        return [app for app in self.applications.values() if app.status == status]
    
    def get_applications_by_company(self, company: str) -> list[SmartApplication]:
        """Get applications for a specific company."""
        return [app for app in self.applications.values() if app.company.lower() == company.lower()]
    
    def get_upcoming_follow_ups(self, days: int = 7) -> list[SmartApplication]:
        ...
        return [
            ...
            if app.follow_up_date and now <= app.follow_up_date <= cutoff
        ]
    
    def get_overdue_follow_ups(self) -> list[SmartApplication]:
    ...
    def set_follow_up(self, application_id: str, days_from_now: int = 7) -> SmartApplication | None:
        """Schedule a follow-up for an application."""
        if application_id not in self.applications:
            return None
        
        app = self.applications[application_id]
        app.follow_up_date = datetime.now() + timedelta(days=days_from_now)
        app.updated_at = datetime.now()
        self._save_applications()
        return app
    
    def add_note(self, application_id: str, note: str) -> SmartApplication | None:
    ...
    def get_statistics(self) -> dict:
        ...
        return {
            "total": total,
            "by_status": {
                status.value: len([app for app in apps if app.status == status])
                for status in ApplicationStatus
            },
            "by_type": {
                app_type.value: len([app for app in apps if app.application_type == app_type])
                for app_type in ApplicationType
            },
            "avg_match_score": sum(app.match_score for app in apps) / total if total > 0 else 0,
            "conversion_rate": self._calculate_conversion_rate(apps),
            "active_applications": len([app for app in apps if app.status not in 
                                       [ApplicationStatus.ARCHIVED, ApplicationStatus.REJECTED]]),
        }
    
    def _calculate_conversion_rate(self, apps: list[SmartApplication]) -> float:
        """Calculate application to interview conversion rate."""
        applied = len([app for app in apps if app.status == ApplicationStatus.APPLIED])
        interviewed = len([app for app in apps if app.status == ApplicationStatus.INTERVIEW])
        
        if applied == 0:
            return 0.0
        return (interviewed / applied) * 100
    ...
    def search_applications(self, query: str) -> list[SmartApplication]:
        ...
        for app in self.applications.values():
            if (query_lower in app.company.lower() or 
                ...
                query_lower in app.notes.lower()):
                results.append(app)
        
        return results


def get_smart_application_tracker(data_dir: Path) -> SmartApplicationTracker:
    """Get or create a smart application tracker instance."""
    return SmartApplicationTracker(data_dir)