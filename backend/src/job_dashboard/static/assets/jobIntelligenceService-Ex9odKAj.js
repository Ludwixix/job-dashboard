...
}`,user:`Generate Recruiter Relationship CRM Briefing for ${a} at ${o}.\n\n${u}`}}},l=async(e,r,a=null)=>{let o=t(),s=o.provider||`openrouter`,l=o.model||`meta-llama/llama-3.3-70b-instruct:free`,u=(o.apiKey||``).trim(),d=o.endpoint||`https://openrouter.ai/api/v1/chat/completions`,f=i[e.toUpperCase()]||{label:e},p=c(e,r,a),m=l;!u&&!m.includes(`:free`)&&s!==`free`&&(m=`meta-llama/llama-3.3-70b-instruct:free`);let h={"Content-Type":`application/json`};u&&(h.Authorization=`Bearer ${u}`),(s===`openrouter`||s===`free`)&&(h[`HTTP-Referer`]=typeof window<`u`?window.location.origin:`https://job-dashboard.app`,h[`X-Title`]=`Career.Agent Intelligence - ${f.label}`);let g=``,_=0,v=0;if(s===`anthropic`&&u){let e=await fetch(d,{method:`POST`,headers:{"Content-Type":`application/json`,"x-api-key":u,"anthropic-version":`2023-06-01`,"anthropic-dangerous-direct-browser-access":`true`},body:JSON.stringify({model:m,system:p.system,messages:[{role:`user`,content:p.user}],max_tokens:3e3,temperature:.2})});if(!e.ok){let t=await e.json().catch(()=>({}));throw Error(t?.error?.message||`Anthropic error HTTP ${e.status}`)}let t=await e.json();g=t?.content?.[0]?.text||`{}`,_=t?.usage?.input_tokens||1e3,v=t?.usage?.output_tokens||500}else{let e=await fetch(d,{method:`POST`,headers:h,body:JSON.stringify({model:m,response_format:{type:`json_object`},messages:[{role:`system`,content:p.system},{role:`user`,content:p.user}],temperature:.2})});if(!e.ok){let t=await e.json().catch(()=>({}));throw Error(t?.error?.message||`LLM Gateway error HTTP ${e.status}`)}let t=await e.json();g=t?.choices?.[0]?.message?.content||`{}`,_=t?.usage?.prompt_tokens||1e3,v=t?.usage?.completion_tokens||500}let y=g.replace(/^```json\s*/i,``).replace(/^```\s*/i,``).replace(/```\s*$/i,``).trim(),b={};try{b=JSON.parse(y)}catch(e){console.warn(`JSON parsing notice, extracting JSON substring:`,e);let t=y.match(/\{[\s\S]*\}/);b=t?JSON.parse(t[0]):{rawResult:y}}return n(m,_,v,f.label),{...b,modelUsed:m,generatedAt:new Date().toISOString()}};export{s as i,o as n,a as r,l as t};