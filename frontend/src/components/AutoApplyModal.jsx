...
${job.coverLetterText || job.coverLetter || ''}`;
 ...
 className="bg-slate-900 border border-amber-500/40 rounded-sm w-full max-w-5xl 2xl:max-w-6xl overflow-hidden flex flex-col max-h-[95vh] text-white font-sans"
 >
 ...
 <div className="p-2.5 rounded-sm bg-amber-600/30 text-amber-300 border border-amber-500/50 ">
 <Zap size={22} className="animate-pulse text-amber-400" />
 </div>
 <div>
 <div className="flex items-center gap-2 mb-0.5">
 <span className="text-[10px] font-mono font-black uppercase tracking-wider px-2 py-0.5 rounded-sm bg-amber-500/20 text-amber-300 border border-amber-400/40">
 ⚡ {platformName}
 </span>
 ...
 {taskStatus === 'running' && <Loader2 size={13} className="animate-spin text-amber-400" />}
 {taskStatus === 'completed' && <CheckCircle2 size={14} className="text-emerald-400" />}
 {phase}
 </span>
 <span className="font-black text-amber-400 text-sm">{progress}%</span>
 </div>
 
 {/* Progress Bar */}
 <div className="h-2 rounded-sm bg-slate-800 overflow-hidden relative">
 <div 
 ...
 <span className="w-2 h-2 rounded-sm bg-emerald-500 animate-ping" />
 LIVE RUNNER ACTIVE
 </span>
 </div>
 {logs.map((l, i) => (
 ...
 <div key={idx} className="p-3.5 rounded-sm bg-slate-950 border border-slate-800 hover:border-slate-700 transition-all">
 <div className="flex items-center justify-between text-[11px] text-amber-400 font-black mb-1">
 ...
 <div className="p-2 rounded-sm bg-slate-900 border border-slate-800 text-xs font-semibold text-emerald-300 flex items-center justify-between">
 <span>{q.answer}</span>
 ...
 className="absolute bottom-6 left-1/2 -translate-x-1/2 px-5 py-3 rounded-sm bg-rose-500 text-white font-bold text-sm flex items-center gap-2 z-[60]"
 >
 <AlertCircle size={16} />
 {toastMsg}
 </motion.div>
