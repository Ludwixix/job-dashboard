...
export const CareerOperations = ({ jobs = [] }) => {
 ...
 <div className="border border-slate-800 bg-slate-900 p-5 rounded-sm">
 <div className="flex items-center gap-2 text-amber-300"><BookmarkPlus size={18} /><h2 className="font-black">Saved search profiles</h2></div>
 <form onSubmit={handleSaveSearch} className="mt-4 grid gap-2">
 <input value={searchName} onChange={(event) => setSearchName(event.target.value)} placeholder="Profile name" className="rounded-sm border border-slate-700 bg-slate-950 px-3 py-2 text-sm" />
 <input value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="Keywords, comma separated" className="rounded-sm border border-slate-700 bg-slate-950 px-3 py-2 text-sm" />
 <button className="inline-flex w-fit items-center gap-2 rounded-sm bg-amber-600 px-3 py-2 text-xs font-bold text-white hover:bg-amber-500"><BookmarkPlus size={14} />Save profile</button>
 </form>
 ...
 <div className="border border-slate-800 bg-slate-900 p-5 rounded-sm"><h2 className="font-black text-sky-300">Source health</h2><div className="mt-3 space-y-2">{sourceHealth.length ? sourceHealth.slice(0, 6).map((check) => <div key={`${check.component}-${check.timestamp}`} className="text-sm"><strong>{check.component.replace('scraper:', '')}</strong><span className={`ml-2 ${check.status === 'healthy' ? 'text-emerald-300' : 'text-amber-300'}`}>{check.status}</span><span className="ml-2 text-slate-500">{check.details.jobs || 0} jobs</span></div>) : <p className="text-sm text-slate-500">No recorded source runs yet.</p>}</div></div>
 </div>
 <div className="border border-slate-800 bg-slate-900 p-5 rounded-sm">
 ...
 <select value={selectedJobId} onChange={(event) => handleExplain(event.target.value)} className="rounded-sm border border-slate-700 bg-slate-950 px-3 py-2 text-sm"><option value="">Select a job</option>{jobs.slice(0, 200).map((job) => <option key={job.id} value={job.id}>{job.title} · {job.company}</option>)}</select>
 {selectedJobId && <button onClick={() => handleFollowUp(jobs.find((job) => job.id === selectedJobId))} className="rounded-sm border border-amber-500/50 px-3 py-2 text-xs font-bold text-amber-300 hover:bg-amber-500/10">Follow up +7d</button>}
 </div>
