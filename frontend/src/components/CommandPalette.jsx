...
export const CommandPalette = ({ 
 ...
 const staticActions = [
 { id: 'view_stream', label: 'View: Live Stream & All Jobs', icon: <Briefcase size={14} className="text-amber-400" />, action: () => onNavigateView('stream') },
 ...
 >
 <div 
 className="bg-slate-900 border border-slate-700/80 rounded-sm max-w-2xl w-full overflow-hidden flex flex-col max-h-[75vh]"
 ...
 />
 <kbd className="hidden sm:inline-block px-2 py-0.5 text-[10px] bg-slate-800 border border-slate-700 text-slate-400 rounded-sm font-mono">ESC</kbd>
 </div>
 ...
 className={`flex items-center justify-between px-3 py-2.5 rounded-sm text-xs font-semibold cursor-pointer transition-colors ${
 isSelected ? 'bg-amber-600 text-white' : 'text-slate-300 hover:bg-slate-800/70'
 }`}
 >
 ...
 className={`flex items-center justify-between px-3 py-2 rounded-sm text-xs cursor-pointer transition-colors ${
 isSelected ? 'bg-slate-800 text-white border border-amber-500/50' : 'text-slate-300 hover:bg-slate-800/40'
 }`}
 >
 <div className="flex items-center gap-3 overflow-hidden">
 <div className="w-7 h-7 rounded-sm bg-slate-800 border border-slate-700 flex items-center justify-center text-[11px] font-bold text-amber-400 shrink-0">
