...
export const CoverLetterPolarizerModal = ({
  ...
  return (
    ...
    >
      <div
        className="bg-slate-900 rounded-3xl shadow-2xl w-full max-w-5xl border border-slate-700/60 flex flex-col max-h-[92vh] overflow-hidden relative"
      ...
      >
        ...
        <div className="bg-slate-950 px-6 py-4 border-b border-slate-800 flex items-center justify-between shrink-0 relative">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-mono px-2 py-0.5 rounded-full bg-slate-800 border border-slate-700 text-slate-300 font-bold">
                  Phase 22
                </span>
              </div>
          ...
          <div className="flex items-center gap-3">
            {currentAudit && (
              <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-xl border bg-slate-900 border-slate-800">
                <span className="text-[10px] font-mono uppercase text-slate-400">Swappability:</span>
                <span className={`text-xs font-black px-2 py-0.5 rounded-md border ${getRiskBadgeColor(currentAudit.swappability_level)}`}>
        ...
        <div className="bg-slate-900/90 border-b border-slate-800 px-6 py-2.5 flex items-center justify-between gap-2 overflow-x-auto shrink-0">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              onClick={handleSave}
              className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-bold rounded-xl border border-slate-700 cursor-pointer flex items-center gap-1.5 transition-colors"
            >
            ...
            <button
              ...
              className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold rounded-xl cursor-pointer flex items-center gap-1.5 shadow-md transition-colors"
            >
              <Download size={14} />
                ...
                <div className="space-y-5">
                  ...
                  <div className="p-5 rounded-2xl bg-gradient-to-r from-slate-950 via-slate-900 to-slate-950 border border-slate-800 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    ...
                    <div className="flex flex-col items-end shrink-0">
                      ...
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${getRiskBadgeColor(currentAudit.swappability_level)}`}>
                        ...
                        {currentAudit.cliches_found.map((c, i) => (
                          <div key={i} className="p-3 rounded-xl bg-slate-950 border border-slate-800 space-y-1">
                            ...
                            <p className="text-[11px] text-slate-300">
                              <strong className="text-indigo-400">Fix:</strong> {c.fix}
                            </p>
                          </div>
                        ))}
                      </div>
                    ...
                    {currentAudit.paragraph_analysis.map((p) => (
                      ...
                      >
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-2 border-b border-slate-800/80 gap-2">
                          <div className="flex items-center gap-2">
                            <span className="w-5 h-5 rounded-full bg-indigo-600/30 text-indigo-400 border border-indigo-500/40 text-[11px] font-mono font-bold flex items-center justify-center">
                              {p.index}
                            </span>
                        ...
                        <p className="text-xs text-slate-300 mt-2 font-mono leading-relaxed bg-slate-900/50 p-2.5 rounded-xl border border-slate-800">
                          {p.preview}
                        </p>

                        <div className="text-[11px] text-slate-400 mt-2 flex items-center gap-1.5">
                          <span className="text-indigo-400 font-bold">Feedback:</span>
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    {variants.map((v) => (
                      ...
                      >
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-2 border-b border-slate-800 gap-2">
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              className="px-3 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-bold flex items-center gap-1 transition-colors cursor-pointer border border-slate-700"
                            >
                        ...
                        <div className="text-[11px] text-indigo-300 font-sans italic">
                          Hook Thesis: {v.hook_explanation}
                        </div>

                        <div className="space-y-2">
                          {v.paragraphs.map((para, pIdx) => (
                            <div key={pIdx} className="p-3 rounded-xl bg-slate-900/60 border border-slate-800/80 text-xs text-slate-300 leading-relaxed font-sans">
              ...
              {activeTab === 'editor' && (
                <div className="space-y-4">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-2 border-b border-slate-800 gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="px-3 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-bold flex items-center gap-1 border border-slate-700 cursor-pointer"
                      >
                  ...
                  <textarea
                    ...
                    className="w-full bg-slate-950 border border-slate-800 rounded-2xl p-4 font-sans text-xs text-slate-200 focus:outline-none focus:border-indigo-500 leading-relaxed resize-y"
                  />
