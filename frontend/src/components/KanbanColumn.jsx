...
const KanbanCard = ({ job, stage, onSelectJob, onOpenCheatSheet, onMoveStage }) => {
 ...
 className={`p-3.5 rounded-sm bg-slate-800/90 border ${needsFollowUp ? 'border-amber-500/60 ring-1 ring-amber-500/30' : 'border-slate-700/80'} cursor-pointer hover:border-indigo-500/60 hover:bg-slate-800 transition-all group select-none active:scale-[0.99] ${isDragging ? 'ring-2 ring-indigo-500 ' : ''}`}
 >
 ...
 className="px-2 py-1 rounded-sm bg-slate-700/70 hover:bg-indigo-600 text-slate-200 hover:text-white text-[10px] font-mono font-bold flex items-center gap-1 transition-colors cursor-pointer"
...
export const KanbanColumn = ({ stage, jobs = [], onSelectJob, onOpenCheatSheet, onMoveStage, className = '' }) => {
 ...
 className={`w-full md:w-auto flex-1 min-w-[260px] md:min-w-[280px] xl:min-w-[300px] max-w-full flex flex-col h-full rounded-sm border ${colors[stage.color]} ${isOver ? 'ring-2 ring-indigo-500/50 bg-slate-850' : ''} transition-all snap-center lg:shrink xl:flex-1 ${className}`}
 >
 ...
 <span
 className={`px-2 py-0.5 rounded-sm text-xs font-mono font-black shadow-xs ${headerColors[stage.color]}`}
