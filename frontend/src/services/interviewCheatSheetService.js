...
export function generateInterviewCheatSheetHtml(job = {}, profileOverride = null, options = {}) {
  ...
  if (options.meetingUrl) meetingInfo.meetingUrl = options.meetingUrl;
  if (options.meetingId) meetingInfo.meetingId = options.meetingId;
  if (options.passcode) meetingInfo.passcode = options.passcode;
  if (options.scheduledTime) meetingInfo.scheduledTime = options.scheduledTime;
  if (options.interviewers && Array.isArray(options.interviewers)) meetingInfo.interviewers = options.interviewers;
  ...
  const numbersToDrop = deriveNumbersToDrop(profile, job);
  const starStories = deriveStarStories(profile, job);
  const reverseQuestions = deriveReverseQuestions(job, meetingInfo);
  const qnaCards = deriveQnACards(job, profile, meetingInfo);
  ...
  const numbersHtml = numbersToDrop.map(num => `
    <div style="background:#f8fafc; padding:6px 8px; border-radius:6px; border:1px solid #e2e8f0;">
      <strong style="color: #0f172a; font-size: 0.95rem;">${num.value}</strong>
      <div style="color:#64748b; font-size: 0.72rem; line-height: 1.2;">${num.label}</div>
    </div>
  `).join('\n');

  // Render Q&A cards markup
  const qnaHtml = qnaCards.map(card => `
    <div class="card qa-card ${card.category}" id="sec-${card.id}">
      <div class="card-header">
        <h3 class="card-title">${card.title}</h3>
        <span class="badge badge-${card.badgeColor}">${card.badge}</span>
      </div>

      <div class="scan-bar ${card.badgeColor === 'green' ? 'green-bar' : card.badgeColor === 'amber' ? 'amber-bar' : card.badgeColor === 'purple' ? 'purple-bar' : ''}">
        <div class="scan-label">${card.scanLabel}</div>
        <div>${card.scanBar}</div>
      </div>

      ${card.likelyQuestion ? `<p style="font-size:0.88rem; color:#475569; margin: 0 0 10px 0;"><strong>${card.likelyQuestion}</strong></p>` : ''}

      <div class="spoken-script">
        <div class="spoken-label">${card.spokenLabel}</div>
        ${card.spokenScript}
      </div>

      <textarea id="${card.notesId}" placeholder="${card.placeholder}"></textarea>
      <span class="save-status" id="${card.statusId}">Saved to Local Storage!</span>
    </div>
  `).join('\n');

  // Render STAR stories markup
  const starStoriesHtml = starStories.map((s, idx) => `
    <div style="background: #f8fafc; border-left: 3px solid ${s.color}; padding: 8px 10px; border-radius: 0 6px 6px 0;">
      <strong style="color: ${s.color}; font-size: 0.82rem;">${idx + 1}. ${s.title}</strong>
      <div style="color: #64748b; font-size: 0.72rem; font-weight: 600; margin-bottom: 3px;">${s.company}</div>
  ...
  const reverseQuestionsHtml = reverseQuestions.map((q, idx) => `
    <div style="background: #f8fafc; border: 1px solid #e2e8f0; padding: 10px; border-radius: 8px;">
      <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 4px;">
        <span class="badge badge-blue" style="font-size: 0.65rem;">${q.category}</span>
        <span style="font-size: 0.7rem; color: #64748b; font-weight: 600;">Target: ${q.targetAudience}</span>
      </div>
    ...
    <aside class="sidebar sidebar-left">
        ...
        <div class="card" style="border-top: 3px solid var(--danger); background: #fffafb; padding: 12px;">
            ...
            <ul style="margin: 0; padding-left: 16px; font-size: 0.78rem; color: #7f1d1d; display:flex; flex-direction:column; gap:6px;">
                ${traps.map(trap => `<li>${trap}</li>`).join('\n')}
            </ul>
        </div>
